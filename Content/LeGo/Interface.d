...
var int Text_PositionX;
var int Text_PositionY;
...
func void PrintS_Ext(var string txt, var int color) {
    ...
    v = Anim8_NewExt(Text_PositionY, gCPrintS_Position, h, false);
    Anim8 (v, Text_PositionY - PF_TextHeight, PF_MoveYTime, A8_SlowEnd);
    p.a8_Movement = v;

    p.tv_Text = Print_Ext(Text_PositionX, Text_PositionY, txt, PF_Font, color, -1);
    p.vr_Pos = Text_PositionY - PF_TextHeight;
    gCPrintS_COff += 1;
    if(!gCPrintS_Act) {
        gCPrintS_COff = 0;
    };
...
instance LevelUpMsg@(LevelUpMsg);

var int LevelUpMsg_Act;
var int LevelUpMsg_COff;

func void LevelUpMsg_Delete(var LevelUpMsg this) {
    Anim8_Delete(this.a8_Movement);
    Print_DeleteText(this.tv_Text);
};

func void LevelUpMsg_Alpha(var int h, var int value) {
    var LevelUpMsg p; p = get(h);
    var zCViewText t; t = get(p.tv_Text);
    t.color = ChangeAlpha(t.color, value);
    if(LevelUpMsg_COff > p.vr_Offs) {
        p.vr_Pos -= (LevelUpMsg_COff - p.vr_Offs) * (PF_TextHeight*2);
        Anim8(p.a8_Movement, p.vr_Pos, PF_MoveYTime, A8_SlowEnd);
        p.vr_Offs = LevelUpMsg_COff;
    };
};

func void LevelUpMsg_Position(var int h, var int value) {
    var LevelUpMsg p; p = get(h);
    var zCViewText t; t = get(p.tv_Text);
    t.posY = value;
};
...
// Softprint
//========================================
func void PrintS_LevelUP(var string txt, var int color) {
    var int h; h = new(LevelUpMsg@);
    var LevelUpMsg p; p = get(h);
    var int v;

    v = Anim8_NewExt(1, LevelUpMsg_Alpha, h, false);
    Anim8_RemoveIfEmpty(v, true);
    Anim8_RemoveDataIfEmpty(v, true);
    Anim8 (v, 255, PF_FadeInTime,  A8_Constant);
    Anim8q(v, 0,   PF_WaitTime,    A8_Wait);
	...
	var int Prinst_Y; Prinst_Y=3600 ;
	
    v = Anim8_NewExt(Prinst_Y, LevelUpMsg_Position, h, false);
    Anim8 (v, Prinst_Y - PF_TextHeight*2, PF_MoveYTime, A8_SlowEnd);
    p.a8_Movement = v;

    p.tv_Text = Print_Ext(-1, Prinst_Y, txt, "FONT_OLD_20_WHITE_HI.tga", color, -1);
    p.vr_Pos = Prinst_Y - PF_TextHeight*2;
    LevelUpMsg_COff += 1;
    if(!LevelUpMsg_Act) {
        LevelUpMsg_COff = 0;
    };
...
func void PrintS2(var string text)
{
PrintS_LevelUP(text, COL_White);
};